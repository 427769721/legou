/* http://localhost:81/manager/page/member/allUsers*/
// http://localhost:81/manager/page/member/allUsers
layui.config({
	base : "../js/"
	...
	//加载页面数据
	var usersData = '';
	$.post("../../member/show", function(data){
		usersData = data;
            	...
            	$.ajax({
					...
					success : function(data){
						...
						for(var i=0;i<usersData.length;i++){
		            		...
		            		//用户名
		            		if(usersStr.uname.indexOf(selectStr) > -1){
			            		usersStr["userName"] = changeStr(usersStr.userName);
		            		}
		            		//用户邮箱
		            		if(usersStr.mailbox.indexOf(selectStr) > -1){
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+  '<td>'+currData[i].mailbox+'</td>'
			    	+  '<td>'+currData[i].gender+'</td>'
			    	+  '<td>'+currData[i].membership+'</td>'
			    	+  '<td>'+currData[i].lasttime+'</td>'
			    	+  '<td>'
					// +    '<a class="layui-btn layui-btn-mini users_edit" data-id="\'+data[i].uid+\'"><i class="iconfont icon-edit"></i> 编辑</a>'
					+    '<a class="layui-btn layui-btn-mini users_edit" data-id="'+data[i].uid+'"><i class="iconfont icon-edit"></i> 编辑</a>'
					+    '<a class="layui-btn layui-btn-danger layui-btn-mini users_del" data-id="'+data[i].uid+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +  '</td>'
			    	+'</tr>';
				}
			}else{
