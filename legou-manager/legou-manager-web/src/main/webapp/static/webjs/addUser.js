	...
	var form = layui.form(),
 	...
 	form.on("submit(addUser)",function(data){
	 	...
	 	if(data.field.userGrade == '0'){
            membership = "非会员";
        }else if(data.field.userGrade == '0'){
            membership = "初级会员";
 		...
 		addUser = '{"usersId":"'+ new Date().getTime().toString() +'",';//id
 		addUser += '"userName":"'+ $(".userName").val() +'",';  //登录名
 		addUser += '"userEmail":"'+ $(".userEmail").val() +'",';	 //邮箱
 		addUser += '"userSex":"'+ data.field.sex +'",'; //性别
 		addUser += '"membership":"'+ userStatus +'",'; //会员等级
 		addUser += '"status":"'+ userGrade +'",'; //会员状态
 		addUser += '"lasttime":"'+ formatTime(new Date()) +'"}';  //登录时间
 		console.log(addUser);
